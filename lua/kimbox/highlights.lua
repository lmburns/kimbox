...
hl.plugins.lsp = {
  ...
  DiagnosticVirtualTextWarn = {
    bg = cfg.diagnostics.background and util.darken(c.yellow, 0.1, c.bg0) or
        c.none,
    fg = c.yellow,
  },
  DiagnosticVirtualTextInfo = {
    bg = cfg.diagnostics.background and util.darken(c.aqua, 0.1, c.bg0) or
        c.none,
    fg = c.aqua,
  },
  DiagnosticVirtualTextHint = {
    bg = cfg.diagnostics.background and util.darken(c.purple, 0.1, c.bg0) or
        c.none,
    fg = c.purple,
  },
...
function M.setup()
        ...
        vim.schedule(
            function()
              vim.notify(
                  ...
                  vim.log.levels.ERROR, { title = "kimbox.nvim" }
              )
            end
        )
        return ""
  ...
  for group_name, group_settings in pairs(vim.g.kimbox_config.highlights) do
    vim.api.nvim_command(
        string.format(
            "highlight %s %s %s %s %s", group_name,
